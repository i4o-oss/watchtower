		...
		try {
			...
			if (formData.headers.trim()) {
				try {
					...
					setFieldErrors({
						headers: 'Invalid JSON format for headers',
					})
					setIsSubmitting(false)
			...
			if (formData.body.trim()) {
				try {
					...
					setFieldErrors({
						body: 'Invalid JSON format for request body',
					})
					setIsSubmitting(false)
					errorToast(
			...
			if (response.ok) {
				successToast(
					'Endpoint Created',
					`Successfully created endpoint "${formData.name}"`,
				)
							...
							<div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
								<div className='space-y-2'>
									...
									<Input
										...
										onBlur={() =>
											handleFieldBlur('timeout_seconds')
										}
										min='1'
										max='300'
										className={
											hasFieldError('timeout_seconds')
												? 'border-red-500 focus:ring-red-500'
												: ''
										}
									/>
